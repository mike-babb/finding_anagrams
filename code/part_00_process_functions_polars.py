...
def split_matrix(
    ...
    if matrix_extraction_option != 1:
        ...
        # word length and letter selector dictionary
...
def generate_from_to_word_group_pairs_simple(
    ...
    for row in curr_wg_df.itertuples(index=False):
        ...
        if n_from_words >= 1:
            ...
            intermediate_to_word_count.update(outcome_word_id_list[:, 0])
    ...
    # outcome_test = intermediate_to_word_count == n_to_word_counter
    # print(outcome_test)
...
def store_anagram_pairs(
    ...
    for i_bp, bp in enumerate(break_point_list[:-1]):
        ...
        # the numpy.int data type is not compatible with sqlite.
        ...
        if next_bp % 10000000 == 0:
            print("...committing changes:", "{:,}".format(next_bp), "records")
            db_conn.commit()
